...
export class UsersController {
  ...
  updateUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (typeof rawBody['password'] === 'string' || typeof rawBody['currentPassword'] === 'string') {
        log.warn('User update blocked: password fields sent to generic update endpoint', {
          requesterId: authReq.user?.id,
          targetUserId: id
        });
        errorResponse(
          res,
          'Password changes are only allowed via /users/:id/change-password (self) or /users/:id/reset-password (admin)',
