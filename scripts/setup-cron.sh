...
SCRIPTS_DIR="${SCRIPTS_DIR:-/opt/event-manager/current/scripts}"
...
# Full backup - Daily at 2 AM
0 2 * * * root ${SCRIPTS_DIR}/backup-full.sh >> /var/log/backup-full.log 2>&1

# Incremental backup - Every 6 hours
0 */6 * * * root ${SCRIPTS_DIR}/backup-incremental.sh >> /var/log/backup-incremental.log 2>&1

# PITR base backup - Weekly on Sunday at 1 AM
0 1 * * 0 root ${SCRIPTS_DIR}/pitr-base-backup.sh >> /var/log/pitr-backup.log 2>&1

# Backup verification - Weekly on Saturday at 3 AM
0 3 * * 6 root ${SCRIPTS_DIR}/backup-verify.sh >> /var/log/backup-verify.log 2>&1

# Backup cleanup - Daily at 4 AM
0 4 * * * root ${SCRIPTS_DIR}/backup-cleanup.sh >> /var/log/backup-cleanup.log 2>&1

# Recovery test - Monthly on 1st at 5 AM
0 5 1 * * root ${SCRIPTS_DIR}/test-recovery.sh >> /var/log/recovery-test.log 2>&1

# System health alert check - Every 5 minutes
*/5 * * * * root ${SCRIPTS_DIR}/ops/system-health-alert.sh >> /var/log/event-manager-health-alert.log 2>&1

# Scoring workflow event alerts - Every 2 minutes
*/2 * * * * root ${SCRIPTS_DIR}/ops/scoring-event-alerts.sh >> /var/log/event-manager-scoring-alert.log 2>&1
