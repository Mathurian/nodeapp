 ...
 * Updated to match current service signatures (tenantId params, findFirst usage)
 */
...
describe('JudgeService', () => {
  ...
  describe('getJudgeIdFromUser', () => {
    it('should return judge ID when user has linked judge', async () => {
      ...
      const result = await service.getJudgeIdFromUser('user-1', tenantId);
    ...
    it('should return null when user has no linked judge', async () => {
      mockPrisma.user.findFirst.mockResolvedValue({ ...mockUser, judge: null } as any);
    ...
    it('should return null when user not found', async () => {
      ...
      const result = await service.getJudgeIdFromUser('invalid-id', tenantId);
  ...
  describe('getStats', () => {
    it('should return judge statistics', async () => {
      ...
      const result = await service.getStats('user-1', tenantId);
    ...
    it('should throw error when user is not linked to judge', async () => {
      ...
      await expect(service.getStats('user-1', tenantId)).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('getAssignments', () => {
    it('should return judge-specific assignments for JUDGE role', async () => {
      ...
      const result = await service.getAssignments('user-1', 'JUDGE', tenantId);

      expect(result).toEqual([mockAssignment]);
      expect(mockPrisma.assignment.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { judgeId: 'judge-1', tenantId }
        })
      );
    });
    ...
    it('should return all assignments for ADMIN role', async () => {
      ...
      const result = await service.getAssignments('user-1', 'ADMIN', tenantId);
    ...
    it('should return all assignments for ORGANIZER role', async () => {
      ...
      const result = await service.getAssignments('user-1', 'ORGANIZER', tenantId);
    ...
    it('should throw error when JUDGE user is not linked to judge record', async () => {
      ...
      await expect(service.getAssignments('user-1', 'JUDGE', tenantId)).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('updateAssignmentStatus', () => {
    it('should update assignment status for ADMIN', async () => {
      ...
      const result = await service.updateAssignmentStatus('assignment-1', 'COMPLETED', 'user-1', 'ADMIN', tenantId);
    ...
    it('should update assignment status for JUDGE who owns the assignment', async () => {
      ...
      const result = await service.updateAssignmentStatus('assignment-1', 'COMPLETED', 'user-1', 'JUDGE', tenantId);

      expect(result).toEqual(mockAssignment);
      expect(mockPrisma.assignment.findFirst).toHaveBeenCalledWith({
        where: { id: 'assignment-1', tenantId }
      });
    });

    it('should throw error when JUDGE does not own the assignment', async () => {
      mockPrisma.user.findFirst.mockResolvedValue(mockUser as any);
      mockPrisma.assignment.findFirst.mockResolvedValue({ ...mockAssignment, judgeId: 'other-judge' } as any);

      await expect(
        service.updateAssignmentStatus('assignment-1', 'COMPLETED', 'user-1', 'JUDGE', tenantId)
    ...
    it('should throw error when assignment not found for JUDGE', async () => {
      ...
      await expect(
        service.updateAssignmentStatus('invalid-id', 'COMPLETED', 'user-1', 'JUDGE', tenantId)
      ).rejects.toThrow(ForbiddenError);
    });
  });
  ...
  describe('getScoringInterface', () => {
    it('should return scoring interface for assigned judge', async () => {
      ...
      const result = await service.getScoringInterface('category-1', 'user-1', tenantId);
    ...
    it('should throw error when user is not linked to judge', async () => {
      ...
      await expect(service.getScoringInterface('category-1', 'user-1', tenantId)).rejects.toThrow(ForbiddenError);
    });
    ...
    it('should throw error when category not found', async () => {
      ...
      await expect(service.getScoringInterface('invalid-id', 'user-1', tenantId)).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('submitScore', () => {
    ...
    it('should create new score when none exists', async () => {
      mockPrisma.user.findFirst.mockResolvedValue(mockUser as any);
      mockPrisma.assignment.findFirst.mockResolvedValue(mockAssignment as any);
      mockPrisma.criterion.findFirst.mockResolvedValue(mockCriterion as any);
    ...
    it('should throw error when criterion not found', async () => {
      mockPrisma.user.findFirst.mockResolvedValue(mockUser as any);
      mockPrisma.assignment.findFirst.mockResolvedValue(mockAssignment as any);
      mockPrisma.criterion.findFirst.mockResolvedValue(null);
    ...
    it('should allow submitting comment without score', async () => {
      ...
      const result = await service.submitScore(
        { categoryId: 'category-1', contestantId: 'contestant-1', comment: 'Comment only', tenantId },
        'user-1'
      );
  ...
  describe('getCertificationWorkflow', () => {
    it('should return certification workflow for assigned judge', async () => {
      ...
      const result = await service.getCertificationWorkflow('category-1', 'user-1', tenantId);
    ...
    it('should throw error when user is not linked to judge', async () => {
      ...
      await expect(service.getCertificationWorkflow('category-1', 'user-1', tenantId)).rejects.toThrow(ForbiddenError);
    });
    ...
    it('should throw error when category not found', async () => {
      ...
      await expect(service.getCertificationWorkflow('invalid-id', 'user-1', tenantId)).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('getContestantBio', () => {
    it('should return single contestant bio for assigned judge', async () => {
      ...
      const result = await service.getContestantBio('contestant-1', 'user-1', tenantId);
    ...
    it('should throw error when contestant not found', async () => {
      ...
      await expect(service.getContestantBio('invalid-id', 'user-1', tenantId)).rejects.toThrow(NotFoundError);
    });
  ...
  describe('getJudgeHistory', () => {
    it('should return judge scoring history', async () => {
      ...
      const result = await service.getJudgeHistory('user-1', tenantId, {});
    ...
    it('should apply pagination parameters', async () => {
      ...
      const result = await service.getJudgeHistory('user-1', tenantId, { page: 2, limit: 25 });
    ...
    it('should filter by category ID', async () => {
      ...
      await service.getJudgeHistory('user-1', tenantId, { categoryId: 'category-1' });
    ...
    it('should filter by event ID', async () => {
      ...
      await service.getJudgeHistory('user-1', tenantId, { eventId: 'event-1' });
    ...
    it('should throw error when user is not linked to judge', async () => {
      ...
      await expect(service.getJudgeHistory('user-1', tenantId, {})).rejects.toThrow(ForbiddenError);
    });
  });
});
