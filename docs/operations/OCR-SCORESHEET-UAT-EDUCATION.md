...
- any phone-photo capture beyond tightly framed, well-lit, low-skew images
- any use of handwritten comments extraction

### When to fall back to delegated entry immediately

- the sheet is from any family other than `Education`
- the upload is cropped, shadowed across the grid, blurred, or noticeably skewed
...
Delegated entry is already implemented and deterministic. For one Education scoresheet, the delegate enters `10` criterion scores once and can proceed through the established attribution and certification workflow.
...
That is not a labor win. It is usually equal to or worse than direct delegated entry, while also carrying extraction risk. Under current UAT results, OCR import does not outperform the shipped fallback.
...
- continue using delegated entry from `TASK-94` as the real fallback for unavailable judges
